...
| `src/35_plot_jsams_revision.py` | Seven figures: two main-paper (`J1`, `J2`), four Supplement (`J3`--`J6`), and the men-and-women gradient panel (`J7`), from scripts 34, 36, 37 and 40; each written as PNG for the sources and vector PDF for the journal's accepted formats, with a deposited manifest of drawn labels that the gates compare against the current tables | Current figure authority |
| `src/39_fetch_fbref_womens_appearances.py` | Women's appearance snapshot assembled from FBref match reports: per-match integrity checks, fixture-coverage and lineup-role gates, and surrogate player identifiers drawn at random | Current women's acquisition authority |
| `src/40_womens_denominator_gradient.py` | The denominator gradient in women's leagues, the men-and-women contrast, and the cross-source agreement check; reuses script 37's estimator unchanged | Current women's diagnostic authority |
| `src/41_womens_injury_record_audit.py` | Matched audit of the public women's injury record against the men's --- presence, recording decay and severity mix --- deposited as aggregates that name nobody | Current outcome-restriction authority |
